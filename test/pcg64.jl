import PcgRandom: Pcg64Random, advance!, backstep!
using Test


@testset "Pcg64RandomTests" begin

    @testset "canon_seed_0_default_stream" begin
        # pcg64 rng(0u); rng();
        canon = UInt64[
        ...
        rng = Pcg64Random(0)
        actual = rand(rng, UInt64, length(canon))
        @test actual == canon
    end

    @testset "canon_seed_42_default_stream" begin
        # pcg64 rng(42u); rng();
        canon = UInt64[
    ...
    @testset "canon_seed_neg1_default_stream" begin
        # pcg64 rng(-1); rng();
        canon = UInt64[
    ...
    @testset "canon_seed_314_default_stream_advance" begin
        ...
        rng = Pcg64Random(314)
        advance!(rng, 6283185307179586476)
        actual = rand(rng, UInt64, length(canon))
        @test actual == canon
    end

    @testset "canon_seed_0_stream_0" begin
        # pcg64 rng(0, 0); rng();
        canon = UInt64[
        ...
        rng = Pcg64Random(0, stream=0)
        actual = rand(rng, UInt64, length(canon))
        @test actual == canon
    end

    @testset "canon_seed_11_stream_6341068275337658369" begin
        ...
        rng = Pcg64Random(11, stream=6341068275337658369)
        actual = rand(rng, UInt64, length(canon))
        @test actual == canon
    end

    @testset "backstep_u64" begin
        ...
        @test result == 5591422465364813936
    end
